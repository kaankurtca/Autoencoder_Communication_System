...
colors = ['b-*','c-*','r-*']
models = ['model_LBC_4_10_1_6dB Burst_AWGN.h5','model_LBC_4_10_2_6dB Burst_AWGN.h5','model_LBC_4_10_3_6dB Burst_AWGN.h5']
...
# Initialize information Data 0/1
    ...
    print('start simulation ...' + str(k) + '_' + str(L) + '_' + str(n[ii]))
    ...
    for Eb_N0_dB in range(0, 20):
        # Noise Variance
        backendNoise = 1 / (2 * R[ii] * 10 ** (Eb_N0_dB / 10))

        # Probability of burst noise
        alpha = 0.05
        burst_beta = np.random.binomial(1, alpha, size=(batch_size, L, 2 * n[ii]))
        ...
        e = Conv1D(filters=2 * n[ii], strides=1, kernel_size=1, name='e_10')(e)  # 2 = I and Q channels
    ...
    # Plot BER Figure
    plt.semilogy(Vec_Eb_N0, Bit_error_rate, colors[ii])
    label = [str(k) + '_' + str(L) + 'n = '+ str(n[ii])]
    plt.legend(label, loc=0)
    plt.xlabel('Eb/N0')
    plt.ylabel('BER')
    plt.title(str(k) + '_' + str(n[ii]) + '_' + str(L))
    plt.grid('true')
plt.show()
